package com.yumeng.libcommon.utils.luban;
...
public class Luban implements Handler.Callback {
  ...
  private File getImageCacheDir(Context context, String cacheName) {
    ...
    if (cacheDir != null) {
      ...
      if(cacheDir1!=null){
        File result = new File(cacheDir1, cacheName);
        if (!result.mkdirs() && (!result.exists() || !result.isDirectory())) {
          // File wasn't able to create a directory, or the result exists but not a directory
          return null;
        }
    ...
    if (Log.isLoggable(TAG, Log.ERROR)) {
      LogUtils.e(TAG, "default disk cache dir is null");
    }
    return null;
  }
  ...
  private void launch(final Context context) {
    ...
    while (iterator.hasNext()) {
      ...
      if (Checker.isImage(path)) {
        ...
        LogUtils.e(TAG, "can not read the path : " + path);
      }
      iterator.remove();
    }
  }
